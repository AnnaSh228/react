...
export default function DisciplineForm() {
  ...
  const saveChanges = async (workId) => {
    setLoading(true);
    try {
      const updatedWorkIndex = laboratoryWorks.findIndex(
        (work) => work.id === workId
      );
  ...
  return (
    <div>
      {loading ? (
          ...
          <table style={{ borderCollapse: "collapse", width: "800px" }}>
            <thead>
              <tr>
                ...
                <th style={{ textAlign: "center", width: "3px" }}>
                  Максимум баллов
                </th>
                <th></th>
              </tr>
              ...
              {laboratoryWorks.map((work, index) => (
                <tr key={work.id}>
                  ...
                  <td
                    ...
                    contentEditable
                    onBlur={(e) =>
                      handleContentEditable(index, "title", e.target.innerText)
                    }
        ...
        <p>Лабораторных работ не найдено</p>
      )}
    </div>
  );
}
